...
let compile filename =
  try
    ...
    | e ->
        Format.eprintf
          "The detection of a combinatorial cycle should raise the \
           Combinatorial_cycle exception; otherwise, fix this one@.";
        raise e);
    close_all ();
    if not !print_only then
      let simulator =
        if !number_steps = -1 then "./prebuilt_netlist_simulator.byte"
        else
